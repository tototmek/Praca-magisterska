...
Przykładem takiego układu odpowiedniego do małych pojemności jest AD7746, zastosowany w pracyn Perraulta \cite{Perrault2016}.
...
Czasy ładowania kondensatorów zapisywane są przez 32-bitowe dodatnie liczby całkowite, co daje wystarczająco duży zakres reprezentacji wyników na potrzeby tworzonego urządzenia.
...
Funkcja ta wyłącza obsługę przerwań mikrokontrolera, których pojawienie się w trakcie akwizycji zakłócałoby sygnał, poprzez zajmowanie czas procesora.
...
Domyślnie skonfigurowane maksimum to 0xffff, czyli zarazem maksymalna wartość licznika iteracji, po której następowałoby przepełnienie.
...
Jego przewagą w tej sytuacji nad innymi podobnymi protokołami (np. SPI, I$^2$C) jest asynchroniczność -- czyli nie wymaga on używania sygnału zegarowego.
...
W przypadku projektowanego systemu konieczne (i, na szczęście, możliwe) jest przesyłanie danych tylko w jednym kierunku -- od układu pomocniczego do głównego.
...
Układ ten realizuje kolejkowanie wysyłanych i odbieranych bajtów, sprawdzanie bitów startu i bitów parzystości, umożliwiając wymianę pojedynczych bajtów informacji w sposób bezbłędny i łatwy w obsłudze.
Pojedyncze bajty nie wystarczają jednak do zapisu pełnej informacji zwrotnej z czujników pojemnościowych: pełna informacja (struktura \texttt{GateReading}) wymaga 9 bajtów.
...
Dane zapisywane są wyłącznie przez mikrokontroler główny, który wykorzystuje je później do algorytmu detekcji pszczół.
...
Przejścia między kolejnymi stanami odbywają się w wyniku odebrania przez kontroler UART kolejnego bajtu. 
Po skutecznym odbiorze sekwencji startowej kolejne bajty zapisywane są do tablicy wynikowej, i inkrementowany jest ich licznik.
...
Wynikowy przebieg sygnału wyjściowego został przedstawiony na rysunku \ref{fig:test-dead-bee}. Dane zostały przefiltrowane średnią kroczącą na oknie 30 próbek.
\begin{figure}[htb]
    \centering
...
\end{figure}
Na ich podstawie jednoznacznie można stwierdzić, że każda z bramek reaguje prawidłowo na pojawienie się pszczoły w tunelu.
...
Przeprowadzone eksperymenty pozwoliły jednoznacznie stwierdzić, że skonstruowane urządzenie działa prawidłowo.
...
Dolna część obudowy wyposażona jest dodatkowo w otwory montażowe, z których użytkownik urządzenia może skorzystać, aby przyczepić je do półki przy wylotku ula.
